...
const WorkItem = ({ title, briefInfo, techs, thumbImages, subTitle, slug}) => {
  ...
  return (
    ...
    <article className={`item-${title} mb-24 flex flex-col gap-6 items-center leading-tight md:mt-12 md:flex-row md:items-start`} id={`item-${title}`}>
        <div className='text-info left-side flex flex-col mt-12 md:mt-6 font-lato text-p-color max-w-[700px] '>
            ...
            <p className='px-[0.2px] font-lato font-bold text-base mt-2 sm:text-[20px] sm:leading-8'>{subTitle}</p>
            <p className='pl-1 text-[15px] font-light leading-[22px] tracking-wide sm:text-[18px] sm:leading-[24px] sm:mt-2'>{briefInfo}</p>
            {techs.length > 0 && ( 
            ...
            <HashLink to={`/works/${slug}`} 
            spy={true} smooth={true} offset={50}className='self-end'> 
