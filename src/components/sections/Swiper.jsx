...
export default ({currentWorkSlug}) => {
  ...
  return (
    <section class='other-work-cta w-11/12 mx-auto mt-4 md:flex md:gap-6 md:justify-start md:mt-20 md:min-h-[440px]'>
      <div>
        <h2 className='font-tektur text-[30px] sm:text-[36px] max-w-[360px] md:text-6xl'>Check Out More Work!</h2>
      </div>
        ...
        {filteredItems.map(( item, index ) => (
          ...
          <ul className='techs mt-2 flex flex-wrap gap-1 font-lato text-sm '>
              {item.techs.map((tech, i) => (
                  <li key={i} className='px-[0.2px] font-extrabold '>{tech} {i < item.techs.length - 1 && <span>|</span>}</li>
              ))}
          </ul>
        )}
          </SwiperSlide>
